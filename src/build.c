...
#include <ctype.h> // added from DBS -Braska
#include <stdio.h>
#include <string.h>
#include <time.h> // added from DBS -Braska
#include <math.h> // added from DBS -Braska
#include <unistd.h>
#include "mud.h"
#include "sha256.h"
...
const char *const r_flags[] = {
   "dark", "death", "nomob", "indoors", "house", "lawful", "neutral", "chaotic",
   ...
   "nosupplicate", "arena", "nomissile", "grav", "htanks", "noyell", "noquit",
   "prototype", "dnd", "_track_", "nowhere", "notrack", "spaceship", "nodriving",
   "timechamber", "noteleport", "noteleportout", "astralshield"
};
...
const char *const o_flags[] = {
    ...
    "antiicer", "antibioandroid", "antiwizard", "donation", "clanobject", 
    "clancorpse", "antivampire", "antidruid", "hidden", "poisoned", "covering", 
    "deathrot", "buried", "prototype", "nolocate", "groundrot", "lootable", 
    "personal", "multi_invoke", "enchanted", "rare", "unique", "antidemon", 
    "antikaio", "noauction","nogive","noquit","nosac", "android_chip",
...
const char *const o_types[] = {
    ...
    "salve", "cook", "keyring", "odor", "chance", "scouter", "dragonball", "dragonradar",
    "piece", "housekey", "mix"
};

const char *const a_types[] = {
   "none", "strength", "dexterity", "intelligence", "wisdom", "constitution",
   "sex", "class", "level", "age", "height", "weight", /*"energy", let's just use mana*/ "hit", "move",
...
const char *const a_flags[] = {
    ...
    "venomshield", "grapple", "powerlevel", "kaioken", "ssj", "ssj2", "ssj3",
    "ssj4", "snamek", "icer2", "icer3", "icer4", "icer5", "pl_switch", 
    "oozaru", "dead", "heart", "tag", "electric_shield", "hyper", "semiperfect",
...
const char *const act_flags[] = {
    ...
    "pacifist", "noattack", "annoying", "statshield", "prototype", "bounty", 
    "robot", "upgrade", "auctioneer", "undertaker", "nofollow", "randomizepl", 
    "rtgshop"
};
...
/* changed "r8" to "" so players on watch can't see it  -- Gorog */
   ...
   "nohomepage", "knowsmystic", "immortality", "r26", "r27", "r28", "r29", "r30", 
   "r31"
};
...
const char *const plr_flags[] = {
    ...
    "flee", "autogold", "automap", "afk", "invisprompt", "compass", "autozeni", 
    "spar", "yellowpk", "redpk", "yellowwar", "redwar", "ghost", "incog", "bounty",
...
int get_areaPlanet( const char *type )
{
    unsigned int i;
...
void do_mset( CHAR_DATA* ch, const char* argument)
{
   ...
   {
      minattr = 1;
      maxattr = 25; // this is 500 in DBS witouth the else statement -Braska
   }
   else
   {
      minattr = 3;
    ...
    if( !str_cmp( arg2, "kairank" ) )
    {
		...
		if( get_trust( ch ) < LEVEL_SUB_IMPLEM ) // Changed from ch->level < 63 -Braska
		{
...
void do_astat( CHAR_DATA* ch, const char* argument)
{
    ...
    ch_printf_color( ch, "&wsoft range: &W%d - %d    &whard range: &W%d - %d\r\n",
                        tarea->low_soft_range, tarea->hi_soft_range, tarea->low_hard_range, tarea->hi_hard_range );
    ch_printf_color( ch, "&wPlanet: %s (%d)\r\n", planet_table[tarea->areaPlanet],tarea->areaPlanet );
...
void do_aset( CHAR_DATA* ch, const char* argument)
{
    ...
    if( arg1[0] == '\0' || arg2[0] == '\0' )
    {
        ...
        send_to_char( "  author credits resetmsg resetfreq flags areaplanet economy\r\n", ch );
