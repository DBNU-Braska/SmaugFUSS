...
#include <sys/types.h> // added from DBS -Braska
#include <stdio.h>
#include <string.h>
#include <time.h> // added from DBS -Braska
#include "mud.h"
#include "bet.h"
    ...
    if( arg2[0] == '\0' )
    {
        if( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
        {
            ...
            */
            ...
            if( obj->item_type == ITEM_DRAGONBALL && IS_NPC( ch ) )
            {
...
void do_give( CHAR_DATA* ch, const char* argument)
{
    ...
    if( is_number( arg1 ) )
    {
        ...
        */
        ...
        amount = atoi( arg1 );
        if( amount <= 0 || ( str_cmp( arg2, "coins" ) && str_cmp( arg2, "coin" ) && str_cmp( arg2, "zeni" ) ) )
        {
        ...
        mudstrlcat( buf, ( amount > 1 ) ? " zeni." : " zeni.", MAX_STRING_LENGTH );
    ...
    if( IS_OBJ_STAT( obj, ITEM_NOGIVE ) )
        {
            send_to_char("You can't give that.\n\r", ch );
            return;
        }
...
bool remove_obj( CHAR_DATA * ch, int iWear, bool fReplace )
{
    ...
    /* Stupid leet stuff, didn't add everything. -Braska
...
void wear_obj( CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, short wear_bit )
{
    ...
    separate_obj( obj );
    /*
    * DBS Powerlevel based wearing -Braska
    */
    if( ch->exp < obj->level )
	{
		...
		send_to_char( buf, ch );
		act( AT_ACTION, "$n tries to use $p, but is too weak.", ch, obj, NULL, TO_ROOM );
		return;
	}
    ...
    /* Old SMAUG level based wear obj. -Braska
    ...
    */
	...
	if( !bypass )
		if ( (IS_OBJ_STAT( obj, ITEM_ONLY_SAIYAN ) && !is_saiyan( ch ) )
			...
			* || ( IS_OBJ_STAT( obj, ITEM_ANTI_HUMAN ) && ( is_human( ch ) || is_kaio( ch ) || is_demon( ch ) ) ) 
			*/
			|| ( IS_OBJ_STAT( obj, ITEM_ANTI_HUMAN ) && is_human( ch ) )
    ...
    /* More leet stuff not adding -Braska
...
void do_wear( CHAR_DATA* ch, const char* argument)
{
    ...
    {
        send_to_char( "You can't do that while in a beserked Oozaru state.\n\r", ch );
        return;
    }
...
void do_sacrifice( CHAR_DATA* ch, const char* argument)
{
	...
	{
		act( AT_PLAIN, "$p cannot be destroyed.", ch, obj, NULL, TO_CHAR );
		return;
	}
    ...
    {
      mudstrlcpy( name, "Braska", 50 ); // I'm porting this, my name! -Braska
    }
    ch->gold += 1;
    if( obj->item_type == ITEM_CORPSE_NPC || obj->item_type == ITEM_CORPSE_PC )
        adjust_favor( ch, 5, 1 );
    ch_printf( ch, "%s gives you one zeni for your sacrifice.\r\n", name );
...
void save_clan_storeroom( CHAR_DATA* ch, CLAN_DATA* clan)
{
	...
	if( !clan )
	{
		bug( "save_clan_storeroom: Null clan pointer!", NULL );
		return;
	}

	if( !ch )
	{
		bug ("save_clan_storeroom: Null ch pointer!", NULL);
		return;
	}

	snprintf( filename, MAX_STRING_LENGTH, "%s%s.vault", CLAN_DIR, clan->filename );
	if( ( fp = fopen( filename, "w" ) ) == NULL )
	{
		bug( "save_clan_storeroom: fopen", NULL );
		perror( filename );
	}
	else
	{
		...
		if( contents )
			fwrite_obj( ch, contents, fp, NULL , OS_CARRY );
		fprintf( fp, "#END\n" );
		ch->level = templvl;
		fclose( fp );
		return;
...
void do_auction( CHAR_DATA* ch, const char* argument)
{
    ...
    if( IS_NPC( ch ) )   // NPC can be extracted at any time and thus can't auction!
        return;
    /* // No level or time restrictions for auctions -Braska
    if( ch->level < 3 )
    {
    ...
    */
    if( arg1[0] == '\0' )
    {
        if( auction->item != NULL )
        {
            ...
            */
            ...
            set_char_color( AT_LBLUE, ch );
            ch_printf( ch,
                "Object '%s' is %s\r\nSpecial properties: %s\r\nIts weight is %d, value is %d, and powerlevel is %d.\r\n",
    ...
    /*		magic_bit_name( obj->magic_flags ), -- currently unused */
            ...
            switch ( obj->item_type )
            {
                ...
                case ITEM_PILL:
                case ITEM_SCROLL:
                case ITEM_POTION: // May have to remove to allow Senzu to work? -Braska
                ...
                case ITEM_SCOUTER:
                    range = obj->value[2];
                    range *= 100;
                    ch_printf( ch, "Scouter pl range is 1 to %s.\r\n", num_punct_ld( range ) );
                break;

                case ITEM_DRAGONBALL:
                    ch_printf( ch, "This is a real dragonball.\r\n" );
                break;
            }
	...
	{
		if( auction->item != NULL )
			ch_printf( ch, "%s is currently being auctioned.\n\r", auction->item->short_descr );
		else
			ch_printf( ch, "Nothing is currently being auctioned.\n\r" );

		ch_printf( ch, "Queued auctions:\n\r" );

		if( auction->freeQueSlot == -1 )
		{
			ch_printf( ch, "No items are currently in the auction que.\n\r" );
			return;
		}
		...
		for( i = 0; i < auction->freeQueSlot ;i++ )
		{
			...
			ch_printf( ch, "[%d] %s starting at %s zeni.\n\r",
    ...
    if( !str_cmp( arg1, "limit" ) )
	{
		...
		ch_printf( ch, "Auction level limit set to %s.\n\r", num_punct_ld( valueld ) );
		return;
	}
    ...
    if( !str_cmp( arg1, "bid" ) )
    {
        if( auction->item != NULL )
        {
            ...
            if( ch->exp < auction->item->level ) //changed from level to exp for pl -Braska
            {
    ...
    /*	    ch_printf( ch, "Bid: %d\r\n",newbet);	*/
            ...
            */
            if( newbet < auction->bet || newbet < ( auction->bet + + ( auction->bet * 0.25 ) )
            {
                ch_printf( ch, "You must bid at least %s zeni over the current bid.\r\n", num_punct( auction->bet * 0.25 ) );
                return;
            }
            ...
            */
            ...
            snprintf( buf, MAX_STRING_LENGTH, "A bid of %s zeni has been received on %s.", num_punct( newbet ), auction->item->short_descr );
            talk_auction( buf );
            return;
        }
        else
    ...
    */
    ...
    if( atoi( arg2 ) < 0 )
    {
        send_to_char( "You can't auction something for less than 1 zeni!\r\n", ch );
        return;
    }
    ...
    if( auction->item == NULL )
    {
        switch ( obj->item_type )
        {
            ...
            */
            ...
            snprintf( buf, MAX_STRING_LENGTH, "A new item is being auctioned: %s at %d zeni.", obj->short_descr, auction->starting );
            talk_auction( buf );
...
const char* get_ed_number( OBJ_DATA* obj, int number ) // added from DBS -Braska
{
	EXTRA_DESCR_DATA* ed;
	int count;
...
const char* get_chance_verb( OBJ_DATA* obj ) // added from DBS -Braska
{
	return( obj->action_desc[0] != '\0' ) ? obj->action_desc : "roll$q" ;
}
