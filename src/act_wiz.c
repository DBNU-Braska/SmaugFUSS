...
const char *const save_flag[] = 
    ...
    "empty", "tmpsave", "r16","r17", "r18", "r19", "r20", "r21", "r22", "r23", "r24",
...
bool add_hiscore_ld( const char * keyword, const char * name, long double score );
 ...
 * Local functions.
 */
long double	atod( const char *string ); // From DBS -Braska
const char * tiny_affect_loc_name( int location ); // From DBS -Braska
ROOM_INDEX_DATA * find_location( CHAR_DATA *ch, const char *arg );
void save_watchlist( void );
void save_banlist( void );  // From DBS -Braska
...
void do_showimmlog( CHAR_DATA *ch, const char *argument )
{
	...
	else
	{
		...
		mudstrlcat( arg, "log", MAX_STRING_LENGTH );
        snprintf( filename, 256, "%s%s", WATCH_DIR, strlower( arg ) );
...
// Added from DBS -Braska
void echo_to_area( AREA_DATA * area, const char *argument )
{
    CHAR_DATA *vic;
...
void do_mstat( CHAR_DATA* ch, const char* argument)
{
    ...
    char ambuf[MAX_STRING_LENGTH]; //added for armour statprintout.. -Braska
    AFFECT_DATA *paf;
    CHAR_DATA *victim;
    SKILLTYPE *skill;
    VARIABLE_DATA *vd;
    ...
    snprintf( ambuf, MAX_STRING_LENGTH, "%d/%d", get_armor( victim ), get_maxarmor( victim ) );
    if( IS_VAMPIRE( victim ) && !IS_NPC( victim ) )
    {
        ...
        pager_printf_color( ch, "&cHps     : &w%-12s    &cBlood  : &w%-12s    &cMove      : &w%-12s    &cArmor      : &w%-12s\r\n",
                            hpbuf, bdbuf, mvbuf, ambuf );
    }
    else
    ...
    pager_printf_color( ch, "&cSpeaks: &w%d   &cSpeaking: &w%d   &cPowerlevel: &w%d",
	...
	if( victim->pcdata && IS_IMMORTAL )
	{
	  	ch_printf( ch,"&cUpdateL: &w%d\n\r", victim->pcdata->upgradeL );
	}
	if( is_kaio( victim ) )
	{
	  	ch_printf( ch,"&cKai rank: &w%d &c(&w%s&c)\n\r", victim->kairank, get_kai( victim ) );
	}
	if( is_demon( victim ) )
        {
          ch_printf( ch,"&cDemon rank: &w%d &c(&w%s&c)\n\r", victim->demonrank, get_demon( victim ) );
        }
        ch_printf( ch,"\n\r" );
	//return; took this out from DBS -Braska
...
void do_minvoke( CHAR_DATA* ch, const char* argument)
{
    ...
    if( !xIS_SET( pMobIndex->act, ACT_PROTOTYPE ) && ch->level < LEVEL_DEMI )
    {
...
void do_oinvoke( CHAR_DATA* ch, const char* argument)
{
    ...
    if( !IS_OBJ_STAT( pObjIndex, ITEM_PROTOTYPE ) && get_trust( ch ) < LEVEL_DEMI )
    {
        send_to_char( "You can't invoke non-prototype objects.\n\r", ch );
        return ;
    }

    if( ( pObjIndex->item_type == ITEM_DRAGONBALL || pObjIndex->item_type == ITEM_DRAGONRADAR ) && get_trust( ch ) < LEVEL_DEMI )
    {
    ...
    */
    if( level <= victim->level )
    {
        ...
        for( sn = 0; sn < num_skills; ++sn )
            victim->pcdata->learned[sn] = 0;
        victim->practice = 0;
        victim->train = 0; //added from DBS -Braska
        victim->hit = victim->max_hit;
        victim->mana = victim->max_mana;
        victim->move = victim->max_move;
        advance_level( victim );
...
void do_immortalize( CHAR_DATA* ch, const char* argument) // did not add the pos to give pretitles automatically -Braska
{
    int i;
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
...
void do_restore( CHAR_DATA* ch, const char* argument)
{
    ...
    else
    {
        ...
        update_pos( victim );
        heart_calc( victim, " " ); // not sure what part of DBS this is from -Braska
        if( ch != victim )
...
void do_destroy( CHAR_DATA* ch, const char* argument)
{
    ...
    */
    ...
    if( !rename( pfile, backup ) )
    {
        ...
        {
            ch_printf( ch, "Unknown error #%d - %s (immortal data). Report to Braska\r\n", errno, strerror( errno ) );
        ...
        for( pArea = first_build; pArea; pArea = pArea->next )
        {
            if( !str_cmp( pArea->filename, areafile ) )
            {
                ...
                {
                ch_printf( ch, "Unknown error #%d - %s (area data). Report to Braska\r\n", errno, strerror( errno ) );
                snprintf( ebuf, MAX_STRING_LENGTH, "%s destroying %s", ch->name, buildfile );
                perror( ebuf );
                }
                break;
    ...
    {
        set_char_color( AT_WHITE, ch );
        ch_printf( ch, "Unknown error #%d - %s. Report to Braska\r\n", errno, strerror( errno ) );
...
void do_pclastrename( CHAR_DATA* ch, const char* argument )
{
	CHAR_DATA *victim;
	char arg1[MAX_INPUT_LENGTH];
	char arg2[MAX_INPUT_LENGTH];
...
int get_pc_race2( const char *type )
{
	int i;
 ...
 */
void do_godset( CHAR_DATA* ch, const char* argument )
{
	char arg1[MAX_INPUT_LENGTH];
	char arg2[MAX_INPUT_LENGTH];
	char arg3[MAX_INPUT_LENGTH];
	...
	{
		lockvictim = TRUE;
        mudstrlcpy( arg1, victim->name, MAX_STRING_LENGTH );
		argument = one_argument( argument, arg2 );
        mudstrlcpy( arg3, argument, MAX_STRING_LENGTH );
	}
	else
	{
		lockvictim = FALSE;
	...
	*/
	...
	if( !str_cmp( arg2, "lname" ) )
	{
		...
		buf2[0] = ' ';
        mudstrlcpy( buf2 + 1, argument, MAX_STRING_LENGTH );
 ...
 */
void do_godstat( CHAR_DATA* ch, const char* argument )
{
	char arg[MAX_STRING_LENGTH];
	CHAR_DATA *victim;
	char buf[MAX_STRING_LENGTH];
	...
	pager_printf_color( ch, "\n\r&G%s%s%s.&C\n\r", victim->name, victim->pcdata->last_name ? victim->pcdata->last_name : "", victim->pcdata->title );
	...
	pager_printf_color( ch, "YEARS: &W%-6d&C        SEX: &W%-7s&C           Log In: %s\r",
	                   get_newage( victim ), victim->sex == SEX_MALE ? "MALE" :
    ...
    mudstrlcpy( buf, num_punct( victim->mana ), MAX_STRING_LENGTH );
    mudstrlcpy( buf2, num_punct( victim->max_mana ), MAX_STRING_LENGTH );
			...
			if( ch->level >= 20 )
			{
				if(paf->modifier == 0 )
					...
					if(paf->modifier > 999 )
						...
						pager_printf( ch, "[%-11.11s;%+-3.3d %7.7s;%5d rds]    ", sktmp->name, paf->modifier, tiny_affect_loc_name(paf->location), paf->duration);
				if(i == 0 )
					i = 1;
				if( (++i % 2 ) == 0 )
					send_to_pager( "\n\r", ch );
...
void do_godwho( CHAR_DATA* ch, const char* argument )
{
	DESCRIPTOR_DATA *d;
	sh_int cswitch = 1;
	CHAR_DATA *vch;
 ...
 */
void do_ghost( CHAR_DATA* ch, const char* argument )
{
	char arg[MAX_INPUT_LENGTH];
	short level;
...
void do_reimburse( CHAR_DATA* ch, const char* argument )
{
	...
	*/
	...
	/* Set a PC's password */ //removed because this needs to be in a diff format... -Braska
	/*
	if( !str_cmp( arg2, "password" ) )
	{
		char *pwdnew;
...
void do_botwatch( CHAR_DATA* ch, const char* argument )
{
	DESCRIPTOR_DATA *d;
	short cswitch = 1;
	CHAR_DATA *vch;
...
void do_pcquesting( CHAR_DATA* ch, const char* argument )
{
	char arg[MAX_INPUT_LENGTH];
	CHAR_DATA *victim;
...
void do_rppedit( CHAR_DATA* ch, const char* argument )
{
	CHAR_DATA *victim;
	char arg1[MAX_INPUT_LENGTH];
	char arg2[MAX_INPUT_LENGTH];
...
void do_dnsusers( CHAR_DATA* ch, const char* argument )
{
        char buf[MAX_STRING_LENGTH];
        DESCRIPTOR_DATA *d;
        int count;
        ...
        for ( d = first_descriptor; d; d = d->next )
        {
			...
			else
			{
							...
							|| ( d->character && !str_prefix( arg, d->character->name ) ) ) )
				{
					...
					pager_printf( ch, " %3d| %3d|%4d| %-12s@%-16s ", d->descriptor,
									d->connected, d->idle / 4, d->original ? d->original->name :
...
void do_nognote( CHAR_DATA* ch, const char* argument )
{
	CHAR_DATA *victim;
	char arg[MAX_INPUT_LENGTH];
	short time;
	...
	ch_printf( victim, "The immortals are not pleased with your actions.\n\r You shall not write gnotes for %d %s%s.\n\r", time, (h_d ? "hour" : "day" ), (time == 1 ? "" : "s" ) );
	save_char_obj( victim );  /* used to save ch, fixed by Thoric 09/17/96 */
	return;
}

void do_viewbuf( CHAR_DATA* ch, const char* argument )
{
    CHAR_DATA *victim;
