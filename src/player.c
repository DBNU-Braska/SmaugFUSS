...
void do_corpse( CHAR_DATA *ch, const char *argument )
{
   char buf[MAX_STRING_LENGTH];
   ...
   if( arg[0] == '\0' )
   {
      ...
      send_to_char( " Type <corpse retrieve> for the service.\n\r", ch );
      return;
   }
   ...
   mudstrlcpy( buf, "the corpse of ", MAX_STRING_LENGTH );
   mudstrlcat( buf, ch->name, MAX_STRING_LENGTH ); 			/* Bug fix here by Samson 12-21-00 See below */
