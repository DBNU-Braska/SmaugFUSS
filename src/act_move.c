...
#include <sys/types.h> // added for DBS -Braska
#include <stdio.h>
#include <stdlib.h> // added for DBS -Braska
#include <string.h>
#include <ctype.h>
#include <time.h> // added for DBS -Braska
#include "mud.h"
...
bool has_exits( ROOM_INDEX_DATA * room ) // added from DBS -Braska
{
    EXIT_DATA *e;
    int count = 0;

    for( e = room->first_exit; e; e = e->next )
	count++;
...
ROOM_INDEX_DATA *generate_exit( ROOM_INDEX_DATA * in_room, EXIT_DATA ** pexit )
{
    ...
    if( in_room->vnum > 32767 )   /* room is virtual */ // Why is this an actual number? in DBS it's MAX_VNUMS -Braska
    {
        serial = in_room->vnum;
        roomnum = in_room->tele_vnum;
        if( ( serial & 65535 ) == orig_exit->vnum ) // Why is this an actual number? in DBS it's MAX_VNUMS -Braska
        {
            brvnum = serial >> 16;
            --roomnum;
            distance = roomnum;
        ...
        {
            brvnum = serial & 65535; // Why is this an actual number? in DBS it's MAX_VNUMS -Braska
            ++roomnum;
...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
    ...
    if( IS_SET( pexit->exit_info, EX_NOMOB ) && IS_NPC( ch ) && !xIS_SET( ch->act, ACT_PET ) && !is_split( ch ) )
    {
    ...
    if( IS_NPC(ch) && xIS_SET( to_room->room_flags, ROOM_NO_MOB ) && !xIS_SET( ch->act, ACT_PET ) && !is_split( ch ) )
    {
        act( AT_PLAIN, "Mobs can't enter there.", ch, NULL, NULL, TO_CHAR );
        return rNONE;
    }
	...
	if( room_HBTC_check( ch, to_room ) )
	{
		char sbuf[MAX_STRING_LENGTH];

		sprintf( sbuf, "You can't enter there until: %24.24s.\n\r",
            ctime( &ch->pcdata->nextHBTCDate ) );
		send_to_char( sbuf, ch );
		return rNONE;
	}

	if( room_HBTC_not_partner( ch, to_room ) )
	{
		send_to_char( "The person in there is not designated as your partner "
