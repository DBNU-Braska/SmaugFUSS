...
#include <time.h>
#include <sys/stat.h>
#include <math.h> // added for DBS -Braska
#include "mud.h"
#include "mapper.h"
#include "sha256.h"
         ...
         case POS_STANDING:
            if( IS_IMMORTAL( victim ) )
               ...
               if( ( victim->in_room->sector_type == SECT_UNDERWATER )
                   ...
                   && !is_icer( victim ) && !is_android( victim ) && !is_bio( victim ) )
...
void do_look( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !IS_NPC( ch )
       && !xIS_SET( ch->act, PLR_HOLYLIGHT ) && !IS_AFFECTED( ch, AFF_TRUESIGHT ) && room_is_dark( ch->in_room ) && !is_android( ch ) && !is_superandroid( ch ) && !is_demon( ch ) )
   {
        ...
        if( str_cmp( arg1, "auto" ) )
			if( ( ship = ship_from_cockpit( ch->in_room->vnum ) ) != NULL )
			{
				...
				if( ship->starsystem )
				{
					...
					{
						pager_printf_color(ch, "&R%s\n\r", missile->missiletype == CONCUSSION_MISSILE ? "A Concusion Missile" :
							( missile->missiletype == PROTON_TORPEDO ? "A Torpedo" : ( missile->missiletype == HEAVY_ROCKET ? "A Heavy Rocket" : "A Heavy Bomb" ) ) );
					}
...
{
    char *argument2; // added from DBS -Braska
   char buf[MAX_STRING_LENGTH];
    char buf2[MAX_STRING_LENGTH];  // added from DBS -Braska
	char kairank[MAX_STRING_LENGTH]; // added from DBS -Braska
	bool kaioshin = FALSE; // added from DBS -Braska
   char clan_name[MAX_INPUT_LENGTH];
   char council_name[MAX_INPUT_LENGTH];
   char invis_str[MAX_INPUT_LENGTH];
    char ghost_str[MAX_INPUT_LENGTH]; // added from DBS -Braska
	char incog_str[MAX_INPUT_LENGTH]; // added from DBS -Braska
   char char_name[MAX_INPUT_LENGTH];
    char last_name[MAX_INPUT_LENGTH] // added from DBS -Braska
   ...
   bool fDeityMatch;
   bool fGroup;
   bool fQuesting; // added from DBS -Braska
   CLAN_DATA *pClan = NULL;
   COUNCIL_DATA *pCouncil = NULL;
   DEITY_DATA *pDeity = NULL;
   FILE *whoout = NULL;
   char clancolor[MAX_INPUT_LENGTH]; //added from DBS -Braska
   ...
   WHO_DATA *next_who = NULL;
    WHO_DATA *first_unknown = NULL; // added from DBS -Braska
   ...
   WHO_DATA *first_skilled = NULL;
    WHO_DATA *first_experienced = NULL; // added from DBS -Braska
	WHO_DATA *first_ultimate = NULL; // added from DBS -Braska
	WHO_DATA *first_veteran = NULL; // added from DBS -Braska
	WHO_DATA *first_fearsome = NULL; // added from DBS -Braska
	WHO_DATA *first_legendary = NULL; // added from DBS -Braska
	WHO_DATA *first_epic = NULL; // added from DBS -Braska
    WHO_DATA *first_ascendant = NULL; // added from DBS -Braska
	WHO_DATA *first_transcendent = NULL; // added from DBS -Braska
	WHO_DATA *first_champion = NULL; // added from DBS -Braska
	WHO_DATA *first_titan = NULL; // added from DBS -Braska
	WHO_DATA *first_mythical = NULL; // added from DBS -Braska
   ...
   fLeader = FALSE;
   fQuesting = FALSE; // added from DBS -Braska
 ...
 // Added race types - hoping I didn't fuck this up because of old races... -Braska
        ...
        d = ( fGroup ? ( whogr ? whogr->d : NULL ) : d->prev ) )
   {
      ...
      if( fShowHomepage && wch->pcdata->homepage && wch->pcdata->homepage[0] != '\0' )
        ...
        if( wch->pcdata->clan )
        {
            ...
            if ( pclan->clan_type == CLAN_GUILD )
                mudstrcpy( clan_name, " <", MAX_INPUT_LENGTH );
            else
                mudstrlcpy( clan_name, " (", MAX_INPUT_LENGTH );
      ...
      if( wch->pcdata->council )
      {
        ...
        {
            if( xIS_SET( wch->act, PLR_WIZINVIS ) )
                snprintf( invis_str, MAX_INPUT_LENGTH, "(i%d) ", wch->pcdata->wizinvis );
            else
				invis_str[0] = '\0';
            if( xIS_SET( wch->act, PLR_GHOST ) )
				sprintf( ghost_str, MAX_INPUT_LENGTH, "(g%d) ", wch->pcdata->ghost_level );
            else
				ghost_str[0] = '\0';
			if( xIS_SET( wch->act, PLR_INCOG ) )
				sprintf( incog_str, MAX_INPUT_LENGTH, "(c%d) ", wch->pcdata->incog_level );
			else
				incog_str[0] = '\0';
        }
        else
        ...
        int bc = snprintf( buf, MAX_STRING_LENGTH, "%*s%s%-15s %s%s%s%s%s%s%s%s%s%s%s%s%s.%s%s%s%s\r\n",
            0, "",
    ...
    */
    ...
    if( first_unknown )
	{
		if( !ch )
			...
			send_to_pager_color( "&z---------------------------------[ &rDEADLY &z]----------------------------------\n\r", ch );
	}
...
void do_where( CHAR_DATA* ch, const char* argument)
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   CHAR_DATA *v; // added from DBS -Braska
   DESCRIPTOR_DATA *d;
   bool found;
   int count = 0; // added from DBS

   one_argument( argument, arg );

   	if( !str_cmp( arg,"npc" ) ) // Split NPCS -Braska
	{
...
bool gTrainSuccess(CHAR_DATA *ch, int stat, short *tAbility)
{
	char buf[MAX_STRING_LENGTH];
	float xp_mod = 0;
...
void do_practice( CHAR_DATA* ch, const char* argument)
{
        ...
        for( sn = 0; sn < num_skills; ++sn )
        {
            ...
            if( skill->type != lasttype )
            {
                ...
                if( skill->type == SKILL_SKILL )
                    pager_printf_color( ch,
                                        " ------------------------------------&CSkills&B-----------------------------------\n\r" );
                else if( skill->type == SKILL_ABILITY )
                    pager_printf_color( ch,
                                        " ----------------------------------&CAbilities&B----------------------------------\n\r" );
                else
                    pager_printf_color( ch,
            ...
            if( ( ch->level < skill->min_level[ch->Class] && skill->skill_level[ch->Class] > ch->exp )
                || ( skill->min_level[ch->Class] == 0 ) || ( skill->skill_level[ch->Class] == 0 )
    ...
    else
    {
        ...
        if( ch->practice <= 0 ) // this may have to be removed in DBS -Braska
        {
        ...
        if( can_prac && ( ( sn == -1 ) || ( !IS_NPC( ch ) && /* ch->level */ ch->exp < skill_table[sn]->skill_level[ch->Class]
            && ch->level < skill_table[sn]->race_level[ch->race] ) ) ) // May need to put min_level[ch->Class] instead -Braska
        {
        ...
        */
        ...
        {
            /* // removed for DBS learning method -Braska
            ch->practice--;
            ...
            */
            ...
            if( sn == gsn_preservation )
			{
				act( AT_TELL, "$n tells you. 'That's as good as it gets!'", mob, NULL, ch, TO_VICT );
			}
...
/*
* Android Updgrade/Practice
*/
void do_upgrade( CHAR_DATA* ch, const char* argument)
{
    CHAR_DATA *mob;
    char buf[MAX_STRING_LENGTH];
    int sn;
    ...
    if( argument[0] == '\0' )
    {
        ...
        for( sn = 0; sn < top_sn; sn++ )
        {
            ...
            if( sn == gsn_instant_trans && ( ( is_android( ch ) // added from DBS -Braska
                || is_superandroid( ch ) ) && !wearing_sentient_chip( ch ) ) )
                continue;
            ...
            if( ch->pcdata->learned[normalSn] > 0 )
                set_pager_color( AT_SCORE, ch );
            pager_printf( ch, " %3.0f%% ", ch->pcdata->learned[normalSn] );
            if( ++col % 3 == 0 )
                send_to_pager( "\r\n", ch );
        }
    ...
    else
    {
        ...
        if( ch->practice <= 0 ) // this may have to be removed in DBS -Braska
        {
            act( AT_TELL, "$n tells you 'You must earn some more upgrade points.'", mob, NULL, ch, TO_VICT );
            return;
        }
        if( IS_IMMORTAL( ch ) )
		{
		...
		if( prac )
		{
            ...
            */
            if( skill_table[sn]->teachers && skill_table[sn]->teachers[0] != '\0' )
            {
                ...
                if( !is_name( buf, skill_table[sn]->teachers ) )
                {
                    act( AT_TELL, "$n tells you, 'I know not know how to make that upgrade.'", 
                    mob, NULL, ch, TO_VICT );
                    return;
                }
            }
            ...
            if( ch->pcdata->learned[sn] >= adept )
            {
                snprintf( buf, MAX_STRING_LENGTH, "$n tells you, 'I've already installed all the data I can for %s.'",
                    skill_table[sn]->name );
                act( AT_TELL, buf, mob, NULL, ch, TO_VICT );
            ...
            else
            {
                ...
                if( sn == gsn_self_destruct )
                {
                    act( AT_TELL, "$n tells you. 'This upgrade comes fully functional out of the box!'",
				       	mob, NULL, ch, TO_VICT );
				}
				else
				{
...
void do_channels( CHAR_DATA* ch, const char* argument)
{
    ...
    if( arg[0] == '\0' )
    {
        ...
        */
        ...
        if( get_trust( ch ) > 2 && !NOT_AUTHED( ch ) )
        {
            ...
            ch_printf_color( ch, "%s", !IS_SET( ch->deaf, CHANNEL_CHAT_OOC ) ? " &G+CHATOOC" : " &g-chatooc" );
        ...
        if(IS_HC( ch ) || IS_IMMORTAL( ch )) 
			ch_printf_color( ch, "%s", !IS_SET( ch->deaf, CHANNEL_HC ) ? " &G+HARD-CORE" : " &g-hard-core" );
		ch_printf_color( ch, "%s", !IS_SET( ch->deaf, CHANNEL_ROLEPLAY ) ? " &G+ROLEPLAY" : " &g-roleplay" );

		send_to_char_color( "\n\r &gPublic channels  (Not Moderated)&G:\n\r  ", ch );
		ch_printf_color( ch, "%s", !IS_SET( ch->deaf, CHANNEL_FOS ) ? " &G+FOS" : " &g-fos" );
        ...
        */
        if( IS_IMMORTAL( ch ) )
        {
            ...
            send_to_char_color( !IS_SET( ch->deaf, CHANNEL_ADMIN ) ? " &G+ADMIN" : " &g-admin", ch );
            send_to_char_color( !IS_SET( ch->deaf, CHANNEL_IMMTALK ) ? " &G+IMMTALK" : " &g-immtalk", ch );
        /*    send_to_char_color( !IS_SET( ch->deaf, CHANNEL_PRAY ) ? " &G+PRAY" : " &g-pray", ch ); */
            ...
            ch_printf_color( ch, "%s", !IS_SET( ch->deaf, CHANNEL_AINFO ) ? " &G+AINFO"	: " &g-ainfo" );
        }
    ...
    */
        ...
        if( ( fClear ) && ( ClearAll ) )
        {
            ...
            REMOVE_BIT( ch->deaf, CHANNEL_CHAT_OOC );
			...
			REMOVE_BIT( ch->deaf, CHANNEL_FOS );
			REMOVE_BIT( ch->deaf, CHANNEL_HC );
        ...
        else if( ( !fClear ) && ( ClearAll ) )
        {
            ...
            SET_BIT( ch->deaf, CHANNEL_CHAT_OOC );
			...
			SET_BIT( ch->deaf, CHANNEL_FOS );
			SET_BIT( ch->deaf, CHANNEL_HC );
...
/*
 * DBS Combat toggle flags -- Melora
 */
...
void do_areas( CHAR_DATA* ch, const char* argument)
{
    ...
    */
    char arg[MAX_STRING_LENGTH];
    int planetSort; // added from DBS -Braska
    bool showPlanetName; // added from DBS -Braska
...
void do_whois( CHAR_DATA* ch, const char* argument)
{
    ...
    pager_printf( ch, "\r\n'%s%s.'\r\n %s is a %s level %d %s %s, %d years of age.\r\n",
        ...
        // Not adding the DBS spouse info -Braska
...
* Sparring Partner 
*/
void do_partner( CHAR_DATA* ch, const char* argument)
{
	CHAR_DATA *victim;
