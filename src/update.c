...
void msdp_update( void ); /* Kavir's Protocol Snippet */
...
void msdp_update( void )
{
   ...
   for( d = first_descriptor; d; d = d->next )
   {
      ...
      if( ch && d->connected == CON_PLAYING && !IS_NPC( ch ) )
      {
         ...
         if ( pOpponent != NULL )
         {
               MSDPSetNumber( d, eMSDP_OPPONENT_HEALTH, pOpponent->hit );
               MSDPSetNumber( d, eMSDP_OPPONENT_HEALTH_MAX, pOpponent->max_hit );
                  ...
                  &&  !IS_SET( pexit->exit_info, EX_DIG ) )
               {
                  ...
                  if( IS_SET( pexit->exit_info, EX_CLOSED ) )
                  {
                     if ( pexit->keyword
                        ...
                        ||    pexit->keyword[0] == '\0' ) )
                     {
                        mudstrlcat( buf, MsdpVar, MAX_STRING_LENGTH );
                        mudstrlcat( buf, dir_name[pexit->vdir], MAX_STRING_LENGTH );
                        mudstrlcat( buf, MsdpVal, MAX_STRING_LENGTH );
         ...
         */
         ...
         for (paf = ch->first_affect; paf; paf = paf->next)
         {
            if ( (skill = get_skilltype( paf->type ) ) != NULL )
            {
               ...
               mudstrlcat( buf, skill_buf, MAX_STRING_LENGTH );
            }
         }
...
void update_handler( void )
{
   ...
   if( --pulse_second <= 0 )
   {
      ...
      char_check(  );
      msdp_update(); /* Kavir's Protocol Snippet */
      check_dns(  );
      reboot_check( 0 );
   }
